...
\textbf{\kaishu{数据集VQA-CP v1上性能对比}}：我们同时将模型LMH-CSS与目前最好的视觉问答模型在数据集VQA-CP v1上进行性能评估。同样，根据骨干网络的不同，我们可以将这些模型分为：（1） \textbf{GVQA}。它是使用模型\textbf{SAN}~\cite{yang2016stacked}模型作为骨干网络。（2）\textbf{AReg}、\textbf{GRL}、\textbf{RUBi}和\textbf{LMH}。这些模型都是使用模型\textbf{UpDn}作为骨干网络。
